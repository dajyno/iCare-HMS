...
async function startServer() {
  ...
  const serviceRoleKey = process.env.SUPABASE_SERVICE_KEY || process.env.VITE_SUPABASE_SERVICE_KEY;
  if (!supabaseUrl || !serviceRoleKey) {
    console.error("Missing Supabase admin credentials — proxy will be unavailable");
  }
  ...
  app.post("/api/supabase-proxy", async (req, res) => {
    try {
      ...
      if (!supabaseAdmin) {
        return res.status(500).json({ error: "Supabase admin client not configured — missing service key" });
      }
